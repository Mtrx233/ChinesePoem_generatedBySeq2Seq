...
def get_key_word(key_list_small,sentence,model):
    ...
    all_key =[]#所有关键字
    for i in sentence :
        if i in key_list_small:
            all_key.append(i)

    temp_key ={}
    for i in key_list_small:
        if i in sentence and len(temp_key)<4:
            temp_key[sentence.find(i)]=i


    for i in sorted(temp_key.keys()):
        key.append(temp_key[i])
...
def get_input(str_input):
    ...
    key_list, all_key = get_key_word(key_list_small, str_input, model)

    # key_list=['风','花','学','月']
    print(key_list)

    char_to_ix = np.load('result1/char_to_ix_{}.npy'.format(epoch), allow_pickle=True).item()
    ix_to_char = np.load('result1/ix_to_char_{}.npy'.format(epoch), allow_pickle=True).item()
    ...
    for i in range(len(all_key)):
        all_key_ix.append(char_to_ix[all_key[i]])
    ...
    return str_input, key_ix, decoder_input, char_to_ix, ix_to_char, all_key_ix


def predict(str_input):
    str_input, key_list, decoder_input, char_to_ix, ix_to_char, all_key = get_input(str_input)

    print(key_list)
    xx = []
    encoder_static = torch.load("result1/encoder_{}.pth".format(epoch)).to('cpu').state_dict()
    decoder_static = torch.load("result1/decoder_{}.pth".format(epoch)).to('cpu').state_dict()
    #3752
    ...
    _, all_key_hidden = encoder(all_key_input, encoder_hidden)
    hidden = 0.85 * hidden + 0.15 * all_key_hidden
    ...
    for di in range(length):
        ...
        best_guess = ix_to_char[prediction.argmax(1).item()]
        if di not in [5, 11, 17, 23, 24]:
            id = di // 6
            if best_guess in str[id*6:len(str)]:
                topv, topi = prediction.data.topk(512)
                best_guess = ix_to_char[topi[0][random.randint(0, 511)].item()]
                while best_guess in ['<START>', '<EOP>', '，',' 。','！','？', '</s>']:
                    best_guess = ix_to_char[topi[0][random.randint(0, 511)].item()]
